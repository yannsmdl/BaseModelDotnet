...
{
    public class UpdateClientCommandHandler : CommandHandler, IRequestHandler<UpdateCommand, ValidationResult>
    {
        ...
        private async Task<ValidationResult> HandlePF(Domain.Entities.Client client, UpdateCommand request)
        {
        ...
        private async Task<ValidationResult> HandlePJ(Domain.Entities.Client client, UpdateCommand request)
        {
             var requiredFields = new (object Field, string ErrorMessage)[]
            {
        ...
        public async Task<ValidationResult> Handle(UpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!await _authenticate.ExistsUserByEmail(request.EmailMain, client.UserId))
            {
                AddError("Usuário invalido");
                return ValidationResult;
            }
            ...
            Guid TenantIdUser = await _authenticate.GetTenantIdByUserId(client.UserId);

            if (TenantIdUser != tenantId)
            {
                AddError("O tenant do usuário não é o mesmo da requisição");
                return ValidationResult;
            }

            await _authenticate.UpdateEmailUser(client.UserId, request.EmailMain);


            if (request.TypeClient == TypeClientEnum.PF)
            {
                var ValidationResultPF = await HandlePF(client, request);
            ...
            {
                var ValidationResultPJ = await HandlePJ(client, request);
